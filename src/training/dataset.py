...
def prepare_dataframe_for_tft(
    ...
    df = prices_df.merge(features_df, on=merge_cols, how='inner', suffixes=("_price", "_feat"))

    # Resolve duplicate OHLCV columns generated by merge
    for column in ["open", "high", "low", "close", "volume", "vwap"]:
        ...
        elif feat_col in df.columns:
            df[column] = df[feat_col]
    ...
    if 'return_5d' not in df.columns:
        if 'close' not in df.columns:
            raise KeyError("Column 'close' required to compute return_5d")
